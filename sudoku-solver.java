 ...
 * Sudoku.java
 * 
 * A Java class that represents and solves a standard 9x9 Sudoku puzzle using recursive backtracking.
 * Originally developed for academic purposes, refactored as a standalone project.
 * 
 * Author: Ethan Quach
 */
...
public class Sudoku {    
    ...
    // Additional fields for tracking row and column constraints
    
    // 2-D array that holds whether a row has a certain value
    private boolean[][] rowHasVal;
    // 2-D array that holds whether a column has a certain value
    ...
    public Sudoku() {
        ...
        // Initialize row and column value trackers
    ...
    private boolean solveRB(int n) {
        ...
        for (int val = 1; val <= 9; val++) {
            if (!(this.subgridHasVal[row/3][col/3][val]) && !(this.rowHasVal[row][val]) && !(this.columnHasVal[col][val])) {
